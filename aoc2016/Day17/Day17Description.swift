...
`hijkl`*`DR`* to get `5745` - all doors closed and locked. However,
going *up* instead is worthwhile: even though it returns you to the room
you started in, your path would then be `DU`, opening a *different set
of doors*.

After going `DU` (and then hashing `hijkl`*`DU`* to get `528e`), only
the right door is open; after going `DUR`, all doors lock. (Fortunately,
